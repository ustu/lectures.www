...
* **View** - преобразует данные в выходной формат (`HTML`, `JSON`, ...),
  обрабатывается представлениями и шаблонами;
* **Controller** - в зависимости от запроса (`URL` схемы, метода `GET`, `POST`)
  выбирает нужные данные и запускает подходящий вид (`View`).
