...
.. seealso::
  ...
  * Сравнение скорости фреймворков и чистого WSGI (http://klen.github.io/py-frameworks-bench).
...
* **Model** - доступ к данным, позволяет извлекать данные и менять их
  состояние;
* **View** - представляет/отображает данные клиенту. В Веб-программировании
  существует ввиде конечных данных (`HTML`, `JSON`, ...), которые получает
...
* **Controller** - отслеживает различные события (действия пользователя) и по
  заданной логике оповещает модель о необходимости изменить состояние системы.
...
.. epigraph::
   ...
   ответ (`Response`) и без них. Нет никакого **"контроллера"** (controller):
   его просто не существует. **"Модель"** (`Model`) же либо представлена
   деревом ресурсов, либо "доменной моделью" (`domain model`) (например,
   моделью :l:`SQLAlchemy`), которая вообще не является частью каркаса. Нам
