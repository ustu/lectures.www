...
.. todo::
   ...
   * Как разделять таблицы по разным метаданным и привязывать к ним свое подключение http://otkds.blogspot.com/2009/04/sqlalchemy.html
...
Поля таблицы хранятся в списке :attr:`sqlalchemy.schema.Table.columns` или его более коротком варианте :attr:`sqlalchemy.schema.Table.c`.
...
Объект класса :class:`sqlalchemy.schema.Table` является частью механизма SQL выражений в :mod:`sqlalchemy` и содержит в себе множество вспомогательных методов для построения SQL запросов:
...
Все таблицы из списка :class:`sqlalchemy.schema.MetaData` можно создать при помощи метода :meth:`sqlalchemy.schema.MetaData.create_all`.
...
Для создания, удаления одной таблицы необходимо использовать методы класса :class:`sqlalchemy.schema.Table`:
...
В :l:`SQLAlchemy` рефлексия означает автоматическую загрузку схемы таблицы из уже существующей базы данных. Реализуется через параметр ``autoload`` в конструкторе класса :class:`sqlalchemy.schema.Table.autoload`.
...
.. literalinclude:: /../sourcecode/6.www.sync/2.codding/9.databases/2.sqlalchemy/2.metadata.py
   :language: python
