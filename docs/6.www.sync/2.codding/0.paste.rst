...
Python `Paste`, или просто `Paste` — набор программ для веб-разработки с
использованием языка Python. Включает в себя множество различных `middleware`,
`WSGI-сервер` и другое. Был разработан `Яном Бикингом`, что бы показать всю
красоту спецификации `WSGI`, которая на тот момент была еще в черновиках.
Проект больше академический и до недавнего времени не имел даже поддержки
`Python 3`, но несмотря на это, многие современные фреймворки взяли за
основу примеры из `Paste` (:l:`TurboGears`, :l:`Zope`, :l:`Pylons`,
:l:`Pyramid`)
