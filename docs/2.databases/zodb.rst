...
ZODB используется в :l:`Zope`, :l:`Plone`, :l:`Zenoss`, :l:`ERP5` и некоторых других системах. Например, :l:`ZODB` и :l:`ZEO` (без :l:`Zope`) используются в системе Indico — программном обеспечении для организации симпозиумов, конференций, лекций и т. п., разработанном и используемом ЦЕРНом. Примером отечественного проекта с использованием этой базы данных является ранняя версия программы электронного документооборота "NauDoc" от компании Naumen.

На данный момент реализована полностью на Python [1]_. Но в зависимостях используется `BTrees <https://pythonhosted.org/BTrees/>`_, который реализован с использованием языка C.
