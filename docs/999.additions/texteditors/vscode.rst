...
директорию. Это необходимо, чтобы в этом каталоге сохранить локальные
...
2. Добавим файл ``.gitignore``:
   ...
   и добавить туда директорию ``hello1_env``, чтобы она не участвовала в
...
чтобы не начинать писать код с нуля. Рассмотрим как создать шаблон с БД
...
конфигурацию для `Pyramid` в файл настроек ``launch.json``, чтобы можно было
